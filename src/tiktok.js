...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function cleanLeadingEmptyNodes(container) {
  if (!container) return;
  ...
  while (container.firstChild) {
    ...
    if (n.nodeType === Node.ELEMENT_NODE) {
      ...
      // <p> rỗng hoặc chỉ &nbsp; hoặc chỉ whitespace → remove
      if (tag === "p") {
        const txt = n.textContent.replace(/\u00A0/g, "").trim();

        // kiểm tra xem p có gì "thật" không
        const hasRealContent = Array.from(n.childNodes).some(ch => {
          if (ch.nodeType === Node.ELEMENT_NODE) {
            const t = ch.tagName.toLowerCase();
            if (["img", "video", "iframe", "svg", "a", "strong", "b", "em", "span"].includes(t))
              return true;
            return ch.textContent.replace(/\u00A0/g, "").trim() !== "";
          }
        ...
        if (!hasRealContent && txt === "") {
          container.removeChild(n);
          continue;
        }
      }

      // Thẻ rỗng (div, span…) → remove nếu KHÔNG chứa img/video/iframe
      if (n.textContent.replace(/\u00A0/g, "").trim() === "") {
        if (!n.querySelector("img,video,iframe,svg")) {
          container.removeChild(n);
          continue;
        }
      }
...
// Xóa phần rác đầu nội dung
cleanLeadingEmptyNodes(drawerBody);
...
function addCustomUI(postUrl, article, postData) {
    ...
    if (existing) {
      DebugLog.add('UI', 'Removing existing UI', { wasVisible });
      existing.remove();
    }
    ...
    toggleBtn.addEventListener('click', toggleUIHandler);
